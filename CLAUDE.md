...
- **AOS is bundled from npm** (styles in `angular.json`, `AOS.init()` in `main.ts` after bootstrap) — the site makes zero external requests. Scroll animations are applied via `data-aos` attributes; component transitions respect `prefers-reduced-motion`.
- **The app is zoneless** (`provideZonelessChangeDetection` in `main.ts`; empty `polyfills` in `angular.json`). `zone.js` exists only as a devDependency because the unit-test builder still injects `zone.js/testing`. Rely on template event bindings/signals/`markForCheck` for change detection — never on zone magic.
- **`/experience` and `/writing` are lazy-loaded** (`loadChildren` + `RouterModule.forChild` in each feature module); only home ships in the initial bundle.
